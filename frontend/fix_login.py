...
replacement = """          <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="mt-4 pt-4 border-t border-gray-200">
               ...
               <div className="grid grid-cols-2 gap-2">
                 <button type="button" onClick={() => { document.querySelector('input[type="email"]').value = 'admin@panchayatguard.gov.in'; document.querySelector('input[type="password"]').value = 'SecurePassword123!'; document.querySelector('form').dispatchEvent(new Event('submit', { cancelable: true, bubbles: true })) }} className="text-xs py-2 bg-blue-50 text-pg-blue font-bold rounded border border-blue-100 hover:bg-blue-100 transition-colors">
                   Login as Admin
                 </button>
                 <button type="button" onClick={() => { document.querySelector('input[type="email"]').value = 'dm.vikram@gov.in'; document.querySelector('input[type="password"]').value = 'SecurePassword123!'; document.querySelector('form').dispatchEvent(new Event('submit', { cancelable: true, bubbles: true })) }} className="text-xs py-2 bg-green-50 text-green-700 font-bold rounded border border-green-100 hover:bg-green-100 transition-colors">
                   Login as Official
                 </button>
               </div>
            </div>
...
content = content.split('<form onSubmit={handleSubmit(onSubmit)} className="space-y-6">')[0] + replacement + "\n        </div>\n      </div>\n    </div>\n  );\n}"
